...
% read log file from calls to bgcF.m
[t,...
 zt,z,ize,izte, ...
 ztt,ze,zte, ...
 Zbuoyancy,Zdrag,Zthrust, ...
...
% problem with log file is that ode45 evaluates F backwards and forwards to get an answer.
% playing back output from ode45 doesn't work for F because of persistent state.  That was
% the previous approach but it did not work.  Unclear how it works with ode45 at all with
% persistent state...  This at least appears to show something close
% to the output in addition to what are probably evaluations before and after.
ii = NaN*tout;
for n=1:length(tout)
    [~,ii(n)] = min(abs(t-tout(n)));
end
[t,...
 zt,z,ize,izte, ...
 ztt,ze,zte, ...
 ...
 Re,zg,S] = deal(...
    t(ii),...
    zt(ii),z(ii),ize(ii),izte(ii), ...
    ztt(ii),ze(ii),zte(ii), ...
    Zbuoyancy(ii),Zdrag(ii),Zthrust(ii), ...
    rho(ii),theta(ii),p(ii), ...
    mf(ii),Vf(ii),thetaf(ii), ...
    alpha(ii),chi(ii),cp(ii), ...
    Re(ii),zg(ii),S(ii));
